...
def caminho(x=0, y=0, pont=0):
    if all(cercado(x, y)) or mat[x][y] == '#':
        ...
        mat[x][y] = "#"
        ponts = [
            0 if cercado(x, y)[0] else caminho(x + 1, y, pont),  # Baixo
            0 if cercado(x, y)[2] else caminho(x - 1, y, pont),  # Cima
            0 if cercado(x, y)[1] else caminho(x, y + 1, pont),  # Direita
            0 if cercado(x, y)[3] else caminho(x, y - 1, pont)  # Esquerda
        ]
